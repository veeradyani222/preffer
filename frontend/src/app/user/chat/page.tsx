...
import { FormEvent, Suspense, useEffect, useMemo, useState, useRef, type ChangeEvent } from 'react';
...
async function uploadPortfolioDocument(portfolioId: string, file: File) {
    const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
    const dataUrl = await readFileAsDataUrl(file);

    const response = await fetch(`${API_URL}/wizard/${portfolioId}/documents`, {
        method: 'POST',
        headers: {
...
function AssistantChatPageContent() {
    ...
    const handleDocumentUpload = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 7.5 * 1024 * 1024) {
            setDocumentStatus('Please upload a document smaller than 7.5 MB.');
            return;
        }
        ...
        try {
            ...
            const statusMessage = documentCount
                ...
                : `Uploaded ${file.name}. I can use it in portfolio edits now.`;
            ...
            setMessages((prev) => [
                ...prev,
                {
                    id: `upload-${Date.now()}`,
                    role: 'assistant',
                    content: `I extracted **${file.name}** and will use it as supporting context while editing this portfolio.`,
                    created_at: new Date().toISOString()
                }
            ]);
        } catch (err: any) {
            setDocumentStatus(err.message || 'Failed to upload document');
        } finally {
        ...
        <div className="flex flex-col h-[calc(100vh-6rem)] -my-6">
            ...
            <div className="flex-none bg-white border-t border-[#E9E9E7] p-6">
                <div className="max-w-3xl mx-auto relative">
                    {isPortfolioChat && (
                        <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-[#E9E9E7] bg-[#F7F7F5] px-4 py-3">
                            <div>
                                <p className="text-sm font-medium text-[#37352f]">Supporting docs for this portfolio</p>
                                <p className="text-xs text-[#9B9A97]">Upload a resume, company profile, brochure, or brief and I’ll use it while editing.</p>
                            </div>
                            <button
                                type="button"
                                onClick={() => documentInputRef.current?.click()}
                                disabled={isUploadingDocument}
                                className="inline-flex w-full shrink-0 items-center justify-center gap-2 rounded-lg border border-[#D9D8D4] bg-white px-3 py-2 text-sm font-medium text-[#37352f] hover:bg-[#FCFCFB] sm:w-auto disabled:cursor-not-allowed disabled:opacity-60"
                            >
                                <FileText size={16} />
                    ...
                    {!isPortfolioChat && selectedChat && (
                        <div className="mb-3 rounded-xl border border-[#E9E9E7] bg-[#F7F7F5] px-4 py-3">
                            <p className="text-sm font-medium text-[#37352f]">Document upload is available in Edit Portfolio chats</p>
                            <p className="text-xs text-[#9B9A97] mt-1">This session is for AI Manager behavior. Use Back, then start or open an Edit Portfolio session to upload supporting docs.</p>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSend} className="relative">
                        <input
                            ...
                            placeholder={isPortfolioChat ? 'Ask to change sections, update text, or redesign...' : 'Instruct your AI manager on how to behave...'}
                    ...
                    {isPortfolioChat && documentStatus && (
                        <p className="mt-2 text-xs text-[#5F5E5B]">{documentStatus}</p>
                    )}
