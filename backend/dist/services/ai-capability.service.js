...
class AICapabilityService {
    ...
    static buildCapabilityReadinessGuidance(configs) {
        const enabled = configs.filter((config) => config.enabled);
        if (!enabled.length) {
            return 'No structured capability tools are enabled.';
        }
        return enabled
            .map((config) => {
            var _a;
            const rules = this.getReadinessRules(enabled, config.capability_key);
            const toolName = ((_a = this.CAPABILITY_TOOL_MAP[config.capability_key]) === null || _a === void 0 ? void 0 : _a[0]) || `create_${config.capability_key}_record`;
            const requirements = [];
            if (rules.requireContact) {
                requirements.push('one contact method (email or phone)');
            }
            if (rules.requiredFieldsAll.length) {
                requirements.push(`all of: ${rules.requiredFieldsAll.join(', ')}`);
            }
            if (rules.requiredFieldsAny.length) {
                requirements.push(`at least one of: ${rules.requiredFieldsAny.join(', ')}`);
            }
            if (rules.minConfidence > 0) {
                requirements.push(`confidence at or above ${Math.round(rules.minConfidence * 100)}%`);
            }
            if (rules.allowHighConfidenceBypassAny && rules.requiredFieldsAny.length) {
                requirements.push(`the "at least one of" requirement may be bypassed only at ${Math.round(rules.highConfidenceThreshold * 100)}% confidence or higher`);
            }
            const readyRule = requirements.length
                ? `ready only when ${requirements.join(', ')}`
                : 'ready when explicit visitor intent is clear from the conversation';
            const leadSpecialCase = config.capability_key === 'lead_capture'
                ? ' Explicit hiring/buying/collaboration intent is ready even without contact.'
                : '';
            return `- ${config.capability_key} (${toolName}): ${readyRule}.${leadSpecialCase} If not ready, ask one concise follow-up for only the missing requirement(s), keep should_execute=false, and list those exact unmet fields in missing_fields.`;
        })
            .join('\n');
    }
...
Readiness requirements by capability:
${this.buildCapabilityReadinessGuidance(enabledConfigs)}
...
- missing_fields must use the exact unmet field names from the readiness requirements above whenever possible.
