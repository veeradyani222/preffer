...
class AIService {
     ...
     */
    static async recommendSections(portfolioType, profession, description, plan = 'free', documentContext) {
        var _a;
        ...
        const totalSections = maxCustomSections + 2;
...
Return JSON:
{
  ...
  "reasoning": "A professional, authoritative explanation citing industry standards."
}
...
Return ONLY valid JSON, no markdown.`;
    ...
    static async draftProfileFromDocuments(portfolioType, existingName, existingProfession, existingDescription, documentContext) {
        logger_1.default.ai('Drafting profile from supporting docs', {
            portfolioType,
            hasExistingName: Boolean(existingName),
            hasExistingProfession: Boolean(existingProfession),
            hasExistingDescription: Boolean(existingDescription),
        });
        const prompt = `You are helping draft the basic onboarding profile for a ${portfolioType === 'company' ? 'business website' : 'personal portfolio'}.

Use only information explicitly present in the user's existing inputs or the uploaded supporting documents.
Do not invent employers, metrics, names, contact details, dates, or credentials.
If a field is already provided and strong, preserve it unless the docs clearly improve it.

Current profile inputs:
- Name: "${existingName || ''}"
- Profession/Industry: "${existingProfession || ''}"
- About: "${existingDescription || ''}"
...
Requirements:
- "description" should be a polished first-person summary for individuals or a polished company summary for businesses.
- Keep "description" between 200 and 900 characters.
- If the documents do not clearly provide a better value for "name" or "profession", keep the current value.
- Never use placeholders.`;
        const text = await (0, gemini_service_1.generateWithFallback)({ temperature: 0.3, maxOutputTokens: 700, responseMimeType: 'application/json' }, prompt);
        const parsed = this.extractJson(text);
        return {
            name: typeof parsed.name === 'string' ? parsed.name.trim() : existingName,
            profession: typeof parsed.profession === 'string' ? parsed.profession.trim() : existingProfession,
            description: typeof parsed.description === 'string' ? parsed.description.trim() : existingDescription,
        };
    }
    /**
...
Return JSON:
{
     ...
     */
    static async improveContent(sectionType, currentContent, feedback, documentContext) {
        var _a, _b;
