...
async function backfill() {
    if (!archestra_agent_service_1.default.isA2AEnabled()) {
    ...
    for (const row of result.rows) {
        ...
        try {
            const agent = await archestra_agent_service_1.default.createAgentOrFallback({
                name: row.name,
            ...
            await database_1.default.query('UPDATE portfolios SET archestra_agent_id = $1 WHERE id = $2', [agent.id, row.id]);
