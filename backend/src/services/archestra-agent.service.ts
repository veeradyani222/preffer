...
function buildCapabilityPolicy(portfolio: PortfolioForPrompt): string {
    ...
    const enabledConfigs = enabledKeys.map((key) => ({
        capability_key: key,
        enabled: true,
        settings_json: capabilityConfig[key]?.settings || capabilityConfig[key]?.settings_json || {},
    }));
    ...
    lines.push(AICapabilityService.buildCapabilityReadinessGuidance(enabledConfigs));
