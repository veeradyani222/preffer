...
export class AICapabilityService {
    ...
    static buildCapabilityReadinessGuidance(configs: CapabilityConfig[]): string {
        ...
        return enabled
            .map((config) => {
                ...
                const toolName = this.CAPABILITY_TOOL_MAP[config.capability_key]?.[0] || `create_${config.capability_key}_record`;
                ...
                if (rules.allowHighConfidenceBypassAny && rules.requiredFieldsAny.length) {
                    requirements.push(
                        `the "at least one of" requirement may be bypassed only at ${Math.round(rules.highConfidenceThreshold * 100)}% confidence or higher`
                    );
                }
