...
export function resolvePortfolioColorScheme(row: any): { name: string; colors: string[] } | null {
    return row?.color_scheme
        || row?.wizard_data?.colorScheme
        || row?.wizard_data?.color_scheme
        || null;
}
