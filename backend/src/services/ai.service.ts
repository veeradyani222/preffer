...
Return ONLY valid JSON, no markdown.`;
    ...
    static async draftProfileFromDocuments(
        portfolioType: 'individual' | 'company',
        existingName: string,
        existingProfession: string,
        existingDescription: string,
        documentContext: string
    ): Promise<{ name: string; profession: string; description: string }> {
        logger.ai('Drafting profile from supporting docs', {
            portfolioType,
...
- Never use placeholders.`;
        ...
        const text = await generateWithFallback(
            { temperature: 0.3, maxOutputTokens: 700, responseMimeType: 'application/json' },
            prompt
        );
        const parsed = this.extractJson(text);
