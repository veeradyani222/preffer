...
// Schema structure for each type
export interface TypeSchema {
    type: SectionType;
    structure: 'object' | 'array' | 'string' | 'array_of_strings';
    fields: {
        name: string;
        description: string;
        required: boolean;
    ...
    example: any;
    mergeHint: string; // Tells AI where to put extra info
}

// All section schemas - Content Writer uses these
export const SECTION_SCHEMAS: Record<SectionType, TypeSchema> = {
    text_block: {
        type: 'text_block',
        structure: 'string',
        fields: [
 ...
 */
export function getSchema(type: SectionType): TypeSchema {
 ...
 */
export function getSchemasForPrompt(): string {
