...
import {
    ...
    buildDocumentContextForSection,
    inferRelevantSectionsFromText,
    WizardDocumentSource,
...
export class WizardController {
    private static buildStoredDocument(
        fileName: string,
        mimeType: string,
        sizeBytes: number,
        ...
        return {
            ...
            previewText,
            relevantSections: inferRelevantSectionsFromText(extractedText),
    ...
    static async updateWizardStep(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            if (isNaN(submittedStep) || submittedStep < 1 || submittedStep > 6) {
                logger.error('Invalid submitted step number', { submittedStep });
                return res.status(400).json({ error: 'Invalid step number (must be 1-6)' });
            }

            const nextStep = submittedStep + 1;

            logger.wizard(submittedStep, 'Saving wizard step data', {
                submittedStep,
                nextStep,
                wizardData,
            });
    ...
    static async recommendSections(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            const documentContext = buildGeneralDocumentContext(
                portfolio.wizard_data?.documentSources,
                { intro: 'SUPPORTING DOCUMENT CONTEXT FOR SECTION RECOMMENDATIONS:' }
            );
    ...
    static async generateContent(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            // Build context
            const wizardData = portfolio.wizard_data || {};
            const documentContext = buildDocumentContextForSection(
    ...
    static async uploadDocument(req: Request, res: Response, next: NextFunction) {
        logger.divider('WIZARD: DOCUMENT EXTRACTION');
        logger.request('POST', `/wizard/${req.params.id}/documents`, {
            fileName: req.body?.fileName,
            mimeType: req.body?.mimeType,
            hasDataUrl: Boolean(req.body?.dataUrl),
        });
        ...
        try {
            ...
            const extracted = await InterfazeService.extractDocumentText({
                fileName,
                mimeType,
                fileData: dataUrl,
            });

            const currentDocs: WizardDocumentSource[] = portfolio.wizard_data?.documentSources || [];
            const nextDoc = WizardController.buildStoredDocument(
                fileName,
                mimeType,
            ...
            await PortfolioService.updateWizard(
                portfolioId,
                userId,
                portfolio.wizard_step,
                { documentSources: mergedDocs }
            );
     ...
     */
    static async scrapeLinkedIn(req: Request, res: Response, next: NextFunction) {
        logger.divider('WIZARD: LINKEDIN SCRAPE');
        logger.request('POST', `/wizard/${req.params.id}/scrape-linkedin`, {
            url: req.body?.url,
        });
        ...
        try {
            ...
            const extracted = await InterfazeService.scrapeLinkedIn(trimmedUrl);

            const currentDocs: WizardDocumentSource[] = portfolio.wizard_data?.documentSources || [];
            const nextDoc = WizardController.buildStoredDocument(
                'LinkedIn Profile',
                'text/html',
                Buffer.byteLength(extracted.extractedText, 'utf8'),
                extracted.extractedText,
                extracted.previewText
            );
     ...
     */
    static async draftProfileFromDocuments(req: Request, res: Response, next: NextFunction) {
        logger.divider('WIZARD: DOCUMENT PROFILE DRAFT');
        logger.request('POST', `/wizard/${req.params.id}/profile-draft`);
        ...
        try {
            ...
            const documentSources: WizardDocumentSource[] = portfolio.wizard_data?.documentSources || [];
            if (!documentSources.length) {
                return res.status(400).json({ error: 'Upload at least one supporting document first' });
            }

            const documentContext = buildGeneralDocumentContext(documentSources, {
            ...
            const wizardData = portfolio.wizard_data || {};
            const draft = await AIService.draftProfileFromDocuments(
                portfolio.portfolio_type,
                wizardData.name || portfolio.name || '',
                wizardData.profession || '',
            ...
            logger.error('Document profile draft failed', error);
            next(error);
        }
    }
